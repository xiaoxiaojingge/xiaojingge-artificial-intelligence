...
import com.bayesserver.statistics.MutualInformation;
...
public class BayesUtil {
    ...
    public static void officialExample() throws IOException, XMLStreamException, InconsistentEvidenceException {
        ...
        // at this point we have fully specified the structural (graphical) specification
        // of the Bayesian Network.

        // We must define the necessary probability distributions for each node.

        // Each node in a Bayesian Network requires a probability distribution conditioned
        // on it's parents.

        // newDistribution() can be called on a Node to create the appropriate probability
        // distribution for a node
        // or it can be created manually.

        // The interface Distribution has been designed to represent both discrete and
        ...
        // The Table class is used to define distributions over a number of discrete
        // variables.

        Table tableA = a.newDistribution().getTable(); // access the table property of the
        // Distribution

        // IMPORTANT
        // Note that calling Node.newDistribution() does NOT assign the distribution to
        // the node.
        // A distribution cannot be assigned to a node until it is correctly specified.
        // If a distribution becomes invalid (e.g. a parent node is added), it is
        // automatically set to null.

        // as node A has no parents there is no ambiguity about the order of variables in
        // the distribution
        ...
        // we could specify the values individually as above, or we can use a
        // TableIterator as follows
        ...
        // Now we will calculate P(A|D=True), i.e. the probability of A given the evidence
        // that D is true

        // use the factory design pattern to create the necessary inference related
        // objects
        ...
        // we could have created these objects explicitly instead, but as the number of
        // algorithms grows
        ...
        inference.query(queryOptions, queryOutput); // note that this can raise an
        // exception (see help for details)
        ...
        queryOptions.setLogLikelihood(true); // only request the log-likelihood if you
        // really need it, as extra computation is
        // involved

        inference.query(queryOptions, queryOutput);
        System.out.println(String.format("P(A|D=True, C=True) = {%s,%s}, log-likelihood = %s.", queryA.get(aTrue),
                queryA.get(aFalse), queryOutput.getLogLikelihood()));

        // Expected output ...
        // P(A|D=True, C=True) = {0.0777777777777778,0.922222222222222}, log-likelihood =
        // -2.04330249506396.
    ...
    public static void testExample() throws XMLStreamException, IOException, InconsistentEvidenceException {
        ...
        SensitivityFunctionOneWay oneWay = sensitivity.oneWay(evidence,
                // 假设节点的状态
                cState1, parameter);
        // 以上代码表示 假设 cState1 ， ParameterReference
        // 为变化参数，计算假设状态如何根据ParameterReference的更改而变化。

        // System.out.println(StrUtil.format("Parameter value = {}",
        // oneWay.getParameterValue()));
        // System.out.println(StrUtil.format("Sensitivity value = {}",
        // oneWay.getSensitivityValue()));
        // System.out.println(StrUtil.format("P(Abnormal | e) = {}",
        // oneWay.getProbabilityHypothesisGivenEvidence()));
        ...
        // System.out.println(StrUtil.format("Gamma = {oneWay.Gamma}",
        // oneWay.getGamma()));
        // System.out.println(StrUtil.format("Eval(0.2) = {}", oneWay.evaluate(0.2)));
        // System.out.println(StrUtil.format("Eval'(0.2) = {}",
        // oneWay.evaluateDeriv(0.2)));
    }
     ...
     * @throws InconsistentEvidenceException
     */
    public static void assosiationAnalysisExample() throws XMLStreamException, IOException, InconsistentEvidenceException {

        // 网络对象
        Network network = new Network("测试贝叶斯网络关联分析");
        ...
        // System.out.println(queryA.get(aState1));
        // System.out.println(queryA.get(aState2));
        // System.out.println(queryA.get(aState3));
        //
        // System.out.println(queryB.get(bState1));
        // System.out.println(queryB.get(bState2));
        //
        ...
        associationOptions.setInferenceFactory(factory);
        AssociationPair associationPair1 = new AssociationPair(aVar, bVar);
        AssociationPair associationPair2 = new AssociationPair(aVar, cVar);
        AssociationOutput calculateResult = Association.calculate(List.of(associationPair1, associationPair2), inference.getEvidence(), associationOptions);
        List<AssociationPairOutput> pairOutputs = calculateResult.getPairOutputs();
        pairOutputs.forEach(pairOutput -> {
            System.out.println(pairOutput.getMutualInformation());
            // System.out.println(pairOutput.getSymmetricMutualInformation());
            // System.out.println(pairOutput.getEntropyX());
            // System.out.println(pairOutput.getEntropyY());
            // System.out.println(pairOutput.getProficiencyYGivenX());
            // System.out.println(pairOutput.getProficiencyXGivenY());
            System.out.println("--------------------------------------");
        });

        // 计算互信息
        Table queryJoint = new Table(aVar, bVar);
        inference.getQueryDistributions().add(queryJoint);
        inference.query(queryOptions, queryOutput);
        // 互信息
        double mutualInformation = MutualInformation.calculate(queryJoint, new VariableContext(aVar), new VariableContext(bVar), LogarithmBase.NATURAL);
        // 熵
        double entropy = Entropy.calculate(queryJoint, LogarithmBase.NATURAL);

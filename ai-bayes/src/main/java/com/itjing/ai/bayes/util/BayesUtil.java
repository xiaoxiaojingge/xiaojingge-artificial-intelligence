...
public class BayesUtil {
    ...
    /**
     * 开始对贝叶斯网络进行分析。
     * 此方法初始化分析上下文，其中包含了网络的节点、链接、条件概率表等信息。
    ...
    public static void startAnalysis(BayesNetwork network) {
        ...
        Map<Integer, BayesNode> nodeMap = network.getNodes().stream().collect(Collectors.toMap(BayesNode::getIndex, node -> node));
        analysisContext.setNodeMap(nodeMap);

        // 构建从节点索引到节点CPT的映射
        Map<Integer, BayesNodeCPT> nodeCPTMap = network.getNodeCPTs().stream().collect(Collectors.toMap(BayesNodeCPT::getIndex, cpt -> cpt));
    ...
    private static BayesNodeEndValTable computeOneNetNode(BayesNode currentNode, AnalysisContext analysisContext) {
        ...
        if (Objects.equals(currentNode.getType(), BayesNodeTypeConstant.NODE_JUDGE)) {
            ...
            double rate = new BigDecimal("100").divide(new BigDecimal(stateCount), 2, RoundingMode.HALF_UP).doubleValue();
    ...
    /**
     * 从非顶部节点的条件概率表中获取状态值列表。
     * 此方法通过遍历当前节点的条件概率表中的每个组合，计算每个状态值的累积概率。
     * 它考虑了当前节点的所有父节点的状态，将父节点的状态概率与当前节点的状态值相乘，
     * 最终得到当前节点在给定父节点状态下的累积概率。
     *
     * @param currentNodeCPT  当前节点的条件概率表。
     * @param analysisContext 分析上下文，包含整个分析过程中的状态值表和其他必要信息。
     * @return 当前节点的状态值列表，表示每个状态的累积概率。
     */
    private static List<Double> getStateValListFromNotTopNode(BayesNodeCPT currentNodeCPT, AnalysisContext analysisContext) {
        // 初始化状态值列表
        List<Double> endValTable = Lists.newArrayList();
        ...
        for (BayesNodeCPT.OneCombin oneCombin : combinList) {
            ...
            for (BayesNodeCPT.StateVal parentsState : parentsStates) {
                ...
                BayesNodeEndValTable bayesNodeEndValTable = analysisContext.getEndValTableList().stream().filter(endVal -> Objects.equals(endVal.getNodeIndex(), fatherNode.getIndex())).findFirst().orElse(null);
                // 如果找到对应的状态值表
            ...
            for (BayesNodeCPT.StateVal currNodeState : currNodeStates) {
                ...
                if (ListUtil.isElementPresent(endValTable, j)) {
                    ...
                    BigDecimal calculateResult = BigDecimal.valueOf(endValTable.get(j)).add(rate.multiply(BigDecimal.valueOf(value)));
        ...
        endValTable = endValTable.stream().map(val -> BigDecimal.valueOf(val).divide(BigDecimal.valueOf(10000)).setScale(2, RoundingMode.HALF_UP).doubleValue()).collect(Collectors.toList());
        return endValTable;
    }
    ...
    /**
     * 获取非顶级节点的状态值列表。
     * 此方法用于分析贝叶斯网络中的非顶级节点，确定其状态值列表。通过检查当前节点的父节点之间是否存在连接，
     ...
     * @return
     */
    private static List<Double> getStateValListForNotTopNode(BayesNodeCPT currentNodeCPT, AnalysisContext analysisContext) {
    ...
    public static List<BayesTipInfo> checkBayesNetwork(BayesNetwork network) {
        ...
        if (CollectionUtils.isEmpty(nodes)) {
            BayesTipInfo tipInfo = getTipInfo(BayesTipInfoTypeEnum.ERROR, BayesNetworkMessageConstant.BAYES_NETWORK_NODE_EMPTY, BayesErrorModifierTypeEnum.MUST_MODIFY);
            result.add(tipInfo);
        } else {
            boolean flag = false;
            // 遍历贝叶斯网络节点
            for (BayesNode node : nodes) {
                ...
                if (Objects.equals(node.getType(), BayesNodeTypeConstant.NODE_OPPORTUNITY)) {
                    if ((toNodeLinks.size() + fromNodeLinks.size()) == 0) {
                        BayesTipInfo tipInfo = getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_NODE_ISOLATED, node.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                        result.add(tipInfo);
                    }
                    if (!flag) {
                        // 检查贝叶斯网络是否有环
                        if (!CollectionUtils.isEmpty(fromNodeLinks)) {
                            ...
                            if (flag) {
                                BayesTipInfo tipInfo = getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_HAVE_LOOP, node.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                                result.add(tipInfo);
                            }
                        }
                    }
                    ...
                    if (count == 0) {
                        BayesTipInfo tipInfo = getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_JUDGE_NODE_LACK_OUTPUT_NODE, node.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                        result.add(tipInfo);
                    }
                } else if (Objects.equals(node.getType(), BayesNodeTypeConstant.NODE_UTILITY)) {
                    BayesNodeCPT nodeCPT = nodeCPTs.stream().filter(cpt -> Objects.equals(cpt.getIndex(), node.getIndex())).findFirst().orElse(null);
                    // 效用节点
                    if (Objects.isNull(nodeCPT)) {
                        BayesTipInfo tipInfo = getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_UTILITY_NODE_LACK_CPT_DATA, node.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                        result.add(tipInfo);
                    }
                    Map<Integer, List<BayesNode>> typeGroup = links.stream().filter(link -> Objects.equals(link.getTo(), node.getIndex())).map(link -> nodes.stream().filter(node1 -> Objects.equals(node1.getIndex(), link.getFrom())).findFirst().orElse(null)).filter(Objects::nonNull).collect(Collectors.groupingBy(BayesNode::getType));
                    // 判断当前效用节点是否有决策节点
                    if (CollectionUtils.isEmpty(typeGroup.get(BayesNodeTypeConstant.NODE_JUDGE))) {
                        BayesTipInfo tipInfo = getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_UTILITY_NODE_LACK_JUDGE_NODE, node.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                        result.add(tipInfo);
                    }
                }
            }
            ;
            // 决策节点的数量
            List<BayesNode> judgeNodeList = nodes.stream().filter(node -> Objects.equals(node.getType(), BayesNodeTypeConstant.NODE_JUDGE)).collect(Collectors.toList());
            ...
            if (judgeNodeCount > 1) {
                ...
                for (BayesNode judgeNode : judgeNodeList) {
                    sign = false;
                    // 查询从当前节点出去的关系
                    List<BayesLink> judgeNodeOutLinks = links.stream().filter(link -> Objects.equals(link.getFrom(), judgeNode.getIndex())).collect(Collectors.toList());
                    // 查询从当前节点进来的关系
                    List<BayesLink> judgeNodeInLinks = links.stream().filter(link -> Objects.equals(link.getTo(), judgeNode.getIndex())).collect(Collectors.toList());
                    long fromJudgeNodeCount = judgeNodeInLinks.stream().map(link -> {
                        BayesNode fromNode = nodes.stream().filter(node -> Objects.equals(node.getIndex(), link.getFrom()) && Objects.equals(node.getType(), BayesNodeTypeConstant.NODE_JUDGE)).findFirst().orElse(null);
                        return fromNode;
                    ...
                    if (!sign) {
                        long toJudgeNodeCount = judgeNodeOutLinks.stream().map(link -> {
                            BayesNode toNode = nodes.stream().filter(node -> Objects.equals(node.getIndex(), link.getTo()) && Objects.equals(node.getType(), BayesNodeTypeConstant.NODE_JUDGE)).findFirst().orElse(null);
                            return toNode;
                        }).filter(Objects::nonNull).count();
                    ...
                    if (!sign) {
                        BayesTipInfo tipInfo = getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_LACK_LINK_BTW_JUDGE_NODES, judgeNode.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                        result.add(tipInfo);
                    }
                }
            }
    ...
    public static boolean checkIfHaveLoopInNet(BayesNode curNode, BayesNode startNode, List<BayesLink> links) {
        ...
        // 筛选出当前节点的所有出链接。
        List<BayesLink> outLinks = links.stream().filter(link -> Objects.equals(link.getFrom(), curNode.getIndex())).collect(Collectors.toList());
    ...
    public static BayesTipInfo checkCPTData(BayesNetwork network, BayesNode checkNode) {
        ...
        List<BayesLink> links = network.getLinks();
        BayesNodeCPT nodeCPT = network.getNodeCPTs().stream().filter(cpt -> Objects.equals(cpt.getIndex(), checkNode.getIndex())).findFirst().orElse(null);
        // 条件概率表不为空
        if (Objects.nonNull(nodeCPT)) {
            if (Objects.nonNull(nodeCPT.getCpt())) {
                List<BayesNode> parentNodeList = links.stream().filter(link -> Objects.equals(link.getTo(), checkNode.getIndex())).map(link -> {
                    BayesNode node = network.getNodes().stream().filter(n -> Objects.equals(n.getIndex(), link.getFrom())).findFirst().orElse(null);
                    return node;
                }).filter(Objects::nonNull).collect(Collectors.toList());
                List<BayesNodeCPT.OneCombin> combinList = nodeCPT.getCpt().getCombinList();
                // 应该的组合数
                Integer shouldCombinCount = parentNodeList.stream().map(node -> node.getStateNameList().size()).reduce(1, (a, b) -> a * b);
                if (combinList.size() < shouldCombinCount) {
                    return getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_CPT_DATA_NOT_COMPLETE, checkNode.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                }
                ...
                if (CollectionUtils.isEmpty(stateVals)) {
                    return getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_LACK_CPT_DATA, checkNode.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
                }
            }
        } else {
            return getTipInfo(BayesTipInfoTypeEnum.ERROR, StrUtil.format(BayesNetworkMessageConstant.BAYES_NETWORK_LACK_CPT_DATA, checkNode.getText()), BayesErrorModifierTypeEnum.MUST_MODIFY);
     ...
     */
    private static BayesTipInfo getTipInfo(BayesTipInfoTypeEnum type, String message, BayesErrorModifierTypeEnum modifierType) {
    ...
    public static void main(String[] args) {
        ...
        aNodeCPT.setIndex(1);
        aNodeCPT.setStateVals(
                List.of(
                        new BayesNodeCPT.StateVal("蓝", 40.00),
                        new BayesNodeCPT.StateVal("橙", 35.00),
        ...
        bNodeCPT.setIndex(2);
        bNodeCPT.setStateVals(
                List.of(
                        new BayesNodeCPT.StateVal("大", 30.00),
                        new BayesNodeCPT.StateVal("小", 70.00)
                )
        );
        ...
        cCPT.setCombinList(
                List.of(
                        new BayesNodeCPT.OneCombin(
                                List.of(
                                        new BayesNodeCPT.StateVal("蓝", null),
                                        new BayesNodeCPT.StateVal("大", null)),
                                List.of(new BayesNodeCPT.StateVal("是", 80.00),
                                        new BayesNodeCPT.StateVal("否", 20.00))),
                        new BayesNodeCPT.OneCombin(
                                List.of(
                                        new BayesNodeCPT.StateVal("蓝", null),
                                        new BayesNodeCPT.StateVal("小", null)),
                                List.of(new BayesNodeCPT.StateVal("是", 60.00),
                                        new BayesNodeCPT.StateVal("否", 40.00))),
                        new BayesNodeCPT.OneCombin(
                                List.of(
                                        new BayesNodeCPT.StateVal("橙", null),
                                        new BayesNodeCPT.StateVal("大", null)),
                                List.of(
                                        new BayesNodeCPT.StateVal("是", 60.00),
                                        new BayesNodeCPT.StateVal("否", 40.00))),
                        new BayesNodeCPT.OneCombin(
                                List.of(
                                        new BayesNodeCPT.StateVal("橙", null),
                        ...
                        new BayesNodeCPT.OneCombin(
                                List.of(
                                        new BayesNodeCPT.StateVal("红", null),
                                        new BayesNodeCPT.StateVal("大", null)),
                                List.of(
                                        new BayesNodeCPT.StateVal("是", 80.00),
                                        new BayesNodeCPT.StateVal("否", 20.00))),
                        new BayesNodeCPT.OneCombin(
                                List.of(
                                        new BayesNodeCPT.StateVal("红", null),
        ...
        dCPT.setCombinList(List.of(new BayesNodeCPT.OneCombin(List.of(new BayesNodeCPT.StateVal("蓝", 40.00), new BayesNodeCPT.StateVal("大", 30.00)), List.of(new BayesNodeCPT.StateVal("是", 80.00), new BayesNodeCPT.StateVal("否", 20.00))), new BayesNodeCPT.OneCombin(List.of(new BayesNodeCPT.StateVal("蓝", 40.00), new BayesNodeCPT.StateVal("小", 70.00)), List.of(new BayesNodeCPT.StateVal("是", 60.00), new BayesNodeCPT.StateVal("否", 40.00))), new BayesNodeCPT.OneCombin(List.of(new BayesNodeCPT.StateVal("橙", 35.00), new BayesNodeCPT.StateVal("大", 30.00)), List.of(new BayesNodeCPT.StateVal("是", 60.00), new BayesNodeCPT.StateVal("否", 40.00))), new BayesNodeCPT.OneCombin(List.of(new BayesNodeCPT.StateVal("橙", 35.00), new BayesNodeCPT.StateVal("小", 70.00)), List.of(new BayesNodeCPT.StateVal("是", 80.00), new BayesNodeCPT.StateVal("否", 20.00))), new BayesNodeCPT.OneCombin(List.of(new BayesNodeCPT.StateVal("红", 25.00), new BayesNodeCPT.StateVal("大", 30.00)), List.of(new BayesNodeCPT.StateVal("是", 80.00), new BayesNodeCPT.StateVal("否", 20.00))), new BayesNodeCPT.OneCombin(List.of(new BayesNodeCPT.StateVal("红", 25.00), new BayesNodeCPT.StateVal("小", 70.00)), List.of(new BayesNodeCPT.StateVal("是", 60.00), new BayesNodeCPT.StateVal("否", 40.00)))));
        ...
        BayesNetwork network = new BayesNetwork(List.of(A, B, C), List.of(link1, link2, link3), List.of(aNodeCPT, bNodeCPT, cNodeCPT));
        ...
        analysisContext.setLinks(network.getLinks());
        analysisContext.setNodeCPTs(network.getNodeCPTs());
        analysisContext.setNodeMap(network.getNodes().stream().collect(Collectors.toMap(BayesNode::getIndex, node -> node)));
        analysisContext.setNodeCPTMap(network.getNodeCPTs().stream().collect(Collectors.toMap(BayesNodeCPT::getIndex, nodeCPT -> nodeCPT)));
        analysisContext.setNodeChildrenMap(network.getLinks().stream().collect(Collectors.groupingBy(BayesLink::getFrom, Collectors.mapping(BayesLink::getTo, Collectors.toSet()))));
        analysisContext.setNodeParentMap(network.getLinks().stream().collect(Collectors.groupingBy(BayesLink::getTo, Collectors.mapping(BayesLink::getFrom, Collectors.toSet()))));
